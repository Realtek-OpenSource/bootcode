...
# * All rights reserved.                                                    
# * 
# * $Header: /usr/local/dslrepos/boot0412/865xboot/crt.s,v 1.1.1.1 2011/06/10 08:06:32 yachang Exp $
# *
# * $Author: yachang $
# *
# * Abstract:
...
# *
# * $Log: crt.s,v $
# * Revision 1.1.1.1  2011/06/10 08:06:32  yachang
# *
# *
...
# *
# * Revision 1.10  2005/12/22 03:07:16  bo_zhao
# * +: add code to support gdb stub
# *
# * Revision 1.9  2005/09/22 05:22:31  bo_zhao
# * *** empty log message ***
# *
...
#*/
	.section ".start", "ax"
#	.text
#	modified by zhaobo for gdb stub
...
1:
	...
	#--- copy handler to default exception vector
	la		$8,0x80000200					# destination start address
	la		$9,__ghsbegin_vectors + 0x200	# source start address
	la		$10,0x80000280					# destination end address
1:
	lw		$11,0($9)
	addiu	$9,4
	sw		$11,0($8)
...
#	#--- save the break service route pointer in ADDRESS	ADD_BP_GDB_STUB_SERVICE
#	la		$8, ADD_BP_GDB_STUB_SERVICE
#	la		$9,	__default_exception_vsr
#	sw		$9, 0($8)
...
2:
	...
	.globl	__rt_arch_hal_save_gp
	.ent	__rt_arch_hal_save_gp
	.set	noreorder
__rt_arch_hal_save_gp:
	sw	gp, 0(v0)
	la	gp, _gp
	jr	ra
	nop
	.set	reorder
	.end	__rt_arch_hal_save_gp

	
	.globl	__rt_arch_hal_restore_gp
	.ent	__rt_arch_hal_restore_gp
	.set	noreorder
__rt_arch_hal_restore_gp:
	lw	gp, 0(v0)
	jr	ra
	nop
	.set	reorder
	.end	__rt_arch_hal_restore_gp



	.globl	__hal_dcache_sync
	.ent	__hal_dcache_sync
	.set	noreorder
__hal_dcache_sync:
	ori	$8, $0, 1
	mtc0	$0, $20
	...
	.set reorder
	.end __hal_dcache_sync


	.globl	__hal_disable_interrupts
	.ent	__hal_disable_interrupts
	.set	noreorder
__hal_disable_interrupts:
	...
	.set reorder
	.end __hal_disable_interrupts


	.globl	__hal_disable_interrupt
	.ent	__hal_disable_interrupt
	.set	noreorder
__hal_disable_interrupt:
	mfc0	$8, $12
	nop
	and		$8, 0XFFFFFFFE
	mtc0	$8, $12
	...
	.set reorder
	.end __hal_disable_interrupt



	.globl	__hal_enable_interrupts
	.ent	__hal_enable_interrupts
	.set	noreorder
__hal_enable_interrupts:
	...
	.set reorder
	.end __hal_enable_interrupts



	.globl	__hal_restore_interrupts
	.ent	__hal_restore_interrupts
	.set	noreorder
__hal_restore_interrupts:
	mfc0	$8, $12
	lw		$9, 0($4)
	and		$8, 0XFFFFFFFE
	or		$8, $8, $9
