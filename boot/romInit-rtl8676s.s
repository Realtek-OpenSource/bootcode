...
#define DRAM166		1
#elif defined(CONFIG_RAM_175)
#define DRAM166		1
#elif defined(CONFIG_RAM_150)
#define DRAM133		1
#else
...
isRTL8676:
	...
	////// SRAM mapping Segment 0 ,8k bytes	//////
	SRAM_Map 0x00600000, 0x6, 0x00, 0x0000	# map 0x80600000 (8K) to SRAM segment 0
	//clean SRAM 
	Mem_Set_Word 0xa0600000,0x2000,0x0  // fill 0
###############################################
	li		t0,0xb8000308		//detect DDR or SDR on board.
	li		t1,0x80000000		//check n31, if 0 equ DDR (default), else SDRAM
	lw		t9,0(t0)
	nop
	and		t1,t1,t9
	bnez	t1,is_SDRAM
...
#ifdef DRAM166
	li		t0,0x70FF8480		//will auto update in auto-detection
	li		t2,0x54A00000		//RX DQS calibration default value, PLL analog calibration value for DDR only.*	.	
#else										//DRAM_133MHz:
	li		t0,0x70FF7480		//will auto update in auto-detection
...
////////////GEN data from 0xa0612000 to 0xa0612FFF, 4k bytes
	li		PATTERN_TEMP1,0xa0612000				//start address
...
//// insert DDR calibration version, record in 0xA0600018 //start///	
	...
	li		DQS_TEMP2,0x1D03					//version: 1.03 , change SRAM size from 16k to 8k
...
////////////Clearn data from 0xa0601000 to 0xa0601FFF, 4k bytes
...
#if 1
//load SPI flash data for check memory controller has crash. ( becouse LX and MC not Async, so modify clcokm maybe has glitch issue ).
	li		t0,0xbfc00000
	lw		t3,0(t0)
	nop
//	SDR & DDR I/O PAD current select for test only. 
...
//enable watch dog
	li		t0,0xb800311c
	li		t1,0x00800000		//enable watchdog and clear up-count counter.
	sw		t1,0(t0)
	nop
