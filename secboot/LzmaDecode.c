...
void lzma_reset(void) {
	huft_flag = 0;
	huft_free_idx = 0;
	huft_free_end = -1;
}

static struct huft *xmalloc(int size)
{
struct huft *huft_p;
...
static void xfree(struct huft *memblk)
{
...
/*
  ...
  //set bank
  oldbank = getbank();
  offset= (unsigned int)ptrSrc - FLASH_START;
  if(offset <0x400000)
	  s29gl128bank(0);
  ...
  currbank = getbank();
  if (currbank == 1) {
	  ptrSrc = (char *)((unsigned int)ptrSrc - 0x400000);
  }*/
  ...
  if (LzmaDecodeProperties(&state.Properties, properties, LZMA_PROPERTIES_SIZE) != LZMA_RESULT_OK)
    return Z_DATA_ERROR;
  state.Probs = (CProb *)xmalloc(LzmaGetNumProbs(&state.Properties) * sizeof(CProb));

  #ifdef _LZMA_OUT_READ
  if (state.Properties.DictionarySize == 0)
    state.Dictionary = 0;
  else
    state.Dictionary = (unsigned char *)xmalloc(state.Properties.DictionarySize);
  #else
  //if (outSizeFull == 0)
  //  outStream = 0;
  //else
  //  outStream = (unsigned char *)xmalloc(outSizeFull);
  #endif
  ...
  if (compressedSize == 0)
    ...
    inStream = (unsigned char *)xmalloc(compressedSize);
  #endif
