...
void processBOOTP (BOOTPpkt * bp)
{
	if (bp->op_type == BOOTP_ETH_REPLY && bp->id == BOOT_RQ_ID
		...
		printf ("BOOTP: get IP %d.%d.%d.%d, file %s from server %d.%d.%d.%d\n\r", 
				(bp->your_ip >> 24) & 0xff,
				(bp->your_ip >> 16) & 0xff,
				(bp->your_ip >> 8) & 0xff,
				bp->your_ip & 0xff, 
		...
		netif.ip = bp->your_ip;
		l = strlen ((const char *)bp->boot_filename);
		if (l > 0 && l < 128) {
			_memset (&bdata, 0, sizeof bdata);
			tftp_req = &bdata;
			tftp_req->server = bp->srv_ip;
			strcpy (tftp_req->file, (const char *)bp->boot_filename);
		}
	}
}
