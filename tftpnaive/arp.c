	...
	for (i = 0; i < MAXARPENTRIES; ++i) {
		if (arpCache[i].ip == p->sender_ipaddr) {
			if (f) {
				...
				_memcpy (arpCache[i].ia, p->sender_hwaddr, 6);
				arpCache[i].ok = f = 1;
			}
		}
	}
...
{
	_memcpy (arpCache[arpCacheCnt].ia, p->sender_hwaddr, 6);
...
int sendARPRequest (unsigned int sender_ip, unsigned int target_ip)
{
	...
	p.protsize = 4;
	p.op = ARP_REQUEST;
	_memcpy (p.sender_hwaddr, netif.IEEEIA, 6);
			...
			if (p->hardsize == 6 && p->protsize == 4) {
				if (p->op == ARP_REQUEST) {
					...
					if (netif.ip == p->target_ipaddr) {
						p->op = ARP_REPLY;
						_memcpy (p->target_hwaddr, p->sender_hwaddr, 10);
						p->sender_ipaddr = netif.ip;
						_memcpy (p->sender_hwaddr, netif.IEEEIA, 6);
						_memcpy (pkt, pkt + 6, 6);
						_memcpy (pkt + 6, netif.IEEEIA, 6);
